...
manager -> backoffice_web_app: Запрашивает статус доставки
backoffice_web_app -> backoffice_bff: Запрашивает статус доставки
note over backoffice_bff: [GET] /api/v1/orders/{id}/status

backoffice_bff -> delivery: Проксирует запрос
note over delivery: [GET] /api/v1/orders/{id}/status

backoffice_web_app <--  delivery: 200 OK
note over backoffice_web_app: Экран "Статус доставки"
manager <--  backoffice_web_app: Статус доставки
```